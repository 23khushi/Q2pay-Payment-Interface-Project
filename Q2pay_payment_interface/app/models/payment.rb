class Payment < ApplicationRecord
  ...
  def amount_cant_be_negative
    errors.add(:amount, "Invalid ") if amount <= 0
  end
  ...
  def self.fetch_index(params, current_user)
    ...
    if params[:minimum].present?
      min_value = Integer(params[:minimum]) rescue nil
      payments = payments.where("amount >= ?", min_value)
    end

    if params[:maximum].present?
      max_value = Integer(params[:maximum]) rescue nil
      payments = payments.where("amount <= ?", max_value)
    end
